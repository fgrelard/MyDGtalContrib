...
int main( int argc, char** argv )
{
        ...
        Board2D board;
        board << SetMode( delta.domain().className(), "BoundingBox" );
                      ...
                      itE = delta.domain().end(); it != itE; ++it )
        {
                ...
                board << CustomStyle( p.className(),
                                      new CustomColors( cmap_grad( v ), cmap_grad( v ) ) )
                      << p;
        ...
        std::cout << endl;
        double initWidth = img.extent()[0];
        double initHeight = img.extent()[1];
        double width, height;
        if (initWidth > initHeight) {
                width = 300;
                height = initHeight * width / initWidth;
        }
        else {
                height = 280;
                width = initWidth * height / initHeight;
        }
        board.saveEPS(outFilename.c_str(), width, height);
        return 0;
}
