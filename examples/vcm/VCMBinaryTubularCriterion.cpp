...
double signedAngleTwoVectors(const Z3i::RealVector& v1,
    ...
    double a1 = signedAngle(v1);
    double a2 = signedAngle(v2);
    double angle = std::max(a1, a2) - std::min(a1, a2);
    if (angle < 0) {
        return 2 * M_PI;
    }
    return angle;
...
template <typename RealVector>
double meanAngle(const std::vector<RealVector>& dirs) {
    std::vector<double> angles;
    for (int i = 0; i < dirs.size(); i++) {
        RealVector f = dirs[i];
        RealVector first = f.getNormalized();
        for (int j = 0; j < dirs.size(); j++) {
            if (i == j) continue;
            RealVector o  = dirs[j];
            RealVector other = o.getNormalized();
...
//   Image volume = VolReader<Image>::importVol(inputFilename);
    ...
    vcm.setMySmallR(r);
    for (const Point& p : setVolume.domain()) {
        DGtal::trace.progressBar(i, setVolume.domain().size());
        i++;
        ...
        for (Matrix m : matrices) {
            vcm_r += m;
            if (m == nil) continue;
            LinearAlgebraTool::getEigenDecomposition(m, evecDirs, evalDirs);
            RealVector dir = evecDirs.column(2);
            dirs.push_back(dir);
        }
