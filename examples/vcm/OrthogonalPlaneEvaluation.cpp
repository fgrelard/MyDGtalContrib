...
int main( int  argc, char**  argv )
{
    ...
    for (auto it = setVolume.begin(), ite = setVolume.end();
        ...
        if (current[2] < 20 || current[2] > 50) continue;
        sliceNumber++;
        if (sliceNumber > 93) break;
        ...
        SubImage image2D = planeProc.sliceFromPlane(volume, 100);
          PGMWriter<SubImage>::exportPGM("/home/florent/trash/slice"+ to_string(sliceNumber) + ".pgm", image2D);
